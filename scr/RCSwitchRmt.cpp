...
bool RCSwitchRmt::disableTransmit()
{
  ...
  if(_tx_encoder){
    ...
    _tx_encoder=nullptr;
#if RMT_OOK_DEBUG
    Serial.println(F("[TX][INI] Encoder TX liberado"));
#endif
  }
  _tx_enabled=false;
  _tx_busy=false;
...
bool RCSwitchRmt::startNextBurst_(){
  if(_q_count==0 || _tx_busy) return false;
...
bool RCSwitchRmt::enableReceive(int pin)
{
...
#if !RMT_OOK_DEBUG
    if (!ensure_service_task_()) {   
    return false;
  }
#endif

  // Si el canal existe y es otro pin → destruye y recrea
  ...
  // Si ya está en el mismo pin → asegurar armado (idempotente)
  if (_rx_ch && _rxPin == pin) {
#if RMT_OOK_DEBUG
...
bool RCSwitchRmt::disableReceive()
{
  if (_rx_ch) {
#if RMT_OOK_DEBUG
    Serial.printf("[RMT-RX] disable on pin=%d\n", _rxPin);
#endif
    rmt_disable(_rx_ch);                   // detener captura en driver nuevo    
    rmt_del_channel(_rx_ch); _rx_ch = nullptr;// para liberar recursos totalmente
  } else {
#if RMT_OOK_DEBUG
    Serial.println("[RMT-RX] disable: no channel (noop)");
#endif
  }
  _rx_enabled = false;
  _cc = 0;
...
bool RCSwitchRmt::create_rx_channel_on_pin_(int pin)
{
  if (_rx_ch && _rxPin == pin) {
#if RMT_OOK_DEBUG
    Serial.printf("[RMT-RX] reuse channel on same pin=%d (enable)\n", pin);
#endif
    ...
    _rxPin = pin; 
   // _rx_need_rearm=false; _rx_ready=false; 
    _cc=0;
    return true;
  }
  ...
  Serial.printf("[RMT-RX] new channel on pin=%d (res=1us, invert_in=%u)\n",
                pin, _cur.inverted ? 0U : 1U);
#endif
  rmt_rx_channel_config_t cfg = {};
  cfg.gpio_num          = (gpio_num_t)pin;
  cfg.clk_src           = RMT_CLK_SRC_DEFAULT;
  cfg.resolution_hz     = 1000000;
  cfg.mem_block_symbols = RX_RAW_BUF_SYMS;        // 64
  cfg.flags.invert_in   = _cur.inverted ? 0U : 1U;
...
bool RCSwitchRmt::ensure_rx_ready_()
{
  ...
  // 2 intentos máx si cae en INVALID_STATE
...
void RCSwitchRmt::refresh_rx_cfg_() {
  ...
  if (_has_fixed_proto) {
    ...
    cfg.signal_range_max_ns = 6'200'000; // 6.2 ms en ns (auto)
  }
